...
const V_cnnmn_credits = `
    ...
    <img src="./cat.jpeg" width="225" height="300">`;
...
const _c$ = document.querySelector.bind(document);
...
let _cnnmn_zones = [];
const _cnnmn_container = _c$("#cnnmnZones")
const featuredContainer = document.getElementById("featuredZones");

async function fetch_sha() {
    ...
    try {
        sharesponse = await fetch(
            "https://api.github.com/repos/gn-math/assets/commits?t=" +
                Date.now()
        );
    } catch (error) {}
...
async function _cnnmn_get_cnnmn_zones() {
	await fetch("cnnmn-games/wrapper.json")
		.then(res => res.json())
		.then(entries => {
			console.log(entries);
			entries.forEach(async (entry) => {
				...
				await fetch(`cnnmn-games/${entry.url}`)
					.then(res => res.text())
					.then(html => {
						//panic!
						for (let i=0;i<100;i++) {
							reportError(`${i} AWUDHAl`)
						}
...
function sortZones() {
	...
	_cnnmn_display_cnnmn_zones(_cnnmn_zones)
    displayZones(zones);
}
...
function displayFeaturedZones(featuredZones) {
    ...
    featuredZones.forEach((file, index) => {
        ...
        const zoneItem = _cnnmn_zone_item(file); // it worked! extracted long code to make it easier to change
    ...
    const imageObserver = _cnnmn_lazy_observer()
...
function _cnnmn_display_cnnmn_zones(cnnmnZones) {
    _cnnmn_container.innerHTML = "";
    cnnmnZones.forEach((file, index) => {
        if (_cnnmn_check_skippable(file)) {
            return;
        }
        const zoneItem = _cnnmn_zone_item(file); // it worked! extracted long code to make it easier to change
        _cnnmn_container.appendChild(zoneItem);
    });
    if (_cnnmn_container.innerHTML === "") {
        _cnnmn_container.innerHTML = "No cnnmn zones found.";
    } else {
        _c$("#cnnmnZonesSummary"
        ).textContent = `cnnmn's zones (${cnnmnZones.length})`;
    }

    const lazyImages = document.querySelectorAll(
        "#cnnmnZones img.lazy-zone-img"
    );
    const imageObserver = _cnnmn_lazy_observer()
...
function _cnnmn_zone_item(file) {
...
function displayZones(zones) {
    ...
    zones.forEach((file, index) => {
        ...
        const zoneItem = _cnnmn_zone_item(file)
                ...
                for (const storeName of storeNames) {
                    ...
                    const p = new Promise((res, rej) => {
                        getAllRequest.onsuccess = () => {
                            result.indexedDB[dbInfo.name][storeName] =
                                sanitizeData(getAllRequest.result, 1000, 100);
                            res();
                        };
...
async function _cnnmn_pick_random() {
    document.getElementById("popupTitle").textContent = "random gaming";
    const popupBody = document.getElementById("popupBody");
    popupBody.innerHTML = `
        <div style="max-height: 60vh; overflow-y: auto;" id="_cnnmn_randomiser_container">
            <h4 id="_cnnmn_randomiser">wait</h4>
            </div>
    `;
    popupBody.contentEditable = false;
    document.getElementById("popupOverlay").style.display = "flex";
    let randomiserx = _c$("#_cnnmn_randomiser");

    let rand = Math.round(Math.random() * zones.length);
    for (let i=0;i<50;i++) {
        rand = Math.round(Math.random() * zones.length);
        randomiserx.innerHTML = `<h4>${zones[rand].name}</h4>`;
        
        console.log(zones[rand].name, i);
        
        await _cnnmn_sleep((i * Math.log(i)) * 2)
    }

    let indiv = _c$("#_cnnmn_randomiser_container");

    let img = document.createElement("img");
    img.id = "_cnnmn_rand_img";
    img.height = 200;
    img.width = 150;
    img.style.borderRadius = "10px";
    img.src = _cnnmn_repcovhtml(zones[rand].cover);

    let playbutton = document.createElement("button");
    playbutton.innerHTML = "play";
    playbutton.onclick = () => {aboutBlank()};

    let reroll = document.createElement("button");
    reroll.innerHTML = "reroll";
    reroll.onclick = () => {_cnnmn_pick_random()};

    indiv.appendChild(img);
    indiv.appendChild(document.createElement("br"));
    
    indiv.appendChild(playbutton);
    indiv.appendChild(reroll);

    document.querySelector("#zoneId").textContent = zones[rand].id;
}



function _cnnmn_sleep(ms) {
